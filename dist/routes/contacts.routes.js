...
router.get('/export', async (req, res) => {
    try {
        ...
        const rows = contacts
            .map((c) => `"${c._id}","${c.username || ''}","${c.name || ''}","${c.email || ''}","${c.phone || ''}","${c.isFollower ? 'Yes' : 'No'}","${(c.tags || []).join(';')}","${c.lastInteractionAt ? c.lastInteractionAt.toISOString() : ''}"`)
            .join('\n');
...
router.post('/:id/tags', async (req, res) => {
    try {
        ...
        const update = {};
        if (addTag)
            update.$addToSet = { tags: addTag };
        if (removeTag)
            update.$pull = { tags: removeTag };
