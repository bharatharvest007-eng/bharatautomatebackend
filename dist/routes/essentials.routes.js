    ...
    try {
        ...
        if (syncMeta) {
            ...
            if (account && account.accessToken) {
                metaResult = await MetaService.setIceBreakers(created.map((ib) => ({ question: ib.question, payload: String(ib.payload || ib.question) })), account.accessToken);
            }
        }
...
router.put('/menu', async (req, res) => {
    try {
        ...
        if (syncMeta) {
            const account = await models.SocialAccount.findOne();
            if (account && account.accessToken) {
                metaResult = await MetaService.setPersistentMenu(created.map((item) => ({
                    title: item.title,
                    url: item.url,
