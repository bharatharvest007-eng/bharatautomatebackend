...
router.put('/', async (req, res) => {
    try {
        ...
        const page = await models.BioPage.findOneAndUpdate({ orgId }, {
            $set: {
                title,
                bio,
...
router.get('/p/:slug', async (req, res) => {
    try {
        const page = await models.BioPage.findOneAndUpdate({ slug: req.params.slug, published: true }, { $inc: { viewCount: 1 } }, { returnDocument: 'after' });
        if (!page)
...
router.post('/track-click', async (req, res) => {
    try {
        ...
        if (pageId) {
            await models.BioPage.updateOne({ _id: pageId, 'blocks.id': blockId }, { $inc: { 'blocks.$.clickCount': 1 } });
        }
