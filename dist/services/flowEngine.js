...
export class FlowEngine {
    ...
    static async processComment(event) {
        ...
        for (const auto of automations) {
            ...
            if (matches) {
                ...
                await models.Automation.updateOne({ _id: auto._id }, {
     ...
     */
    static async executeAutomationSteps(automation, event, account) {
        const steps = automation.steps || [];
        for (const step of steps) {
            // Action 1: Public Comment Auto-Reply
            if (step.type === 'comment_reply') {
                const variants = step.variants || [step.text || 'Check your DMs! 🚀'];
            ...
            if (step.type === 'ask_for_follow') {
                const isFollowing = await MetaService.checkFollowerStatus(account.instagramBusinessId, event.userIgsid, account.accessToken);
                if (!isFollowing) {
        ...
        await models.Contact.findOneAndUpdate({ accountId: account._id, igsid: event.userIgsid }, {
            $set: {
